...
test = Warfy()
test.load_var('DUST_1')
test.load_var('HGT')
check = test.get_var('DUST_1').isel(time=10)
check.values[check.values<1e-5]= np.nan
...
check.sel(zlevel=1)
z = check.zlevel.values
ax.set_zticks(z)
ax.set_zlim(0,32)
LON, LAT = np.meshgrid(check.lon.values,check.lat.values)
im = ax.contourf(LAT,LON,test.get_var('HGT').isel(time=0).values,offset=0,cmap='terrain')

for i in range(32):
    if np.isnan(check.sel(zlevel=i).values).all():
        print('zlevel {:} empty!'.format(i))
    else:
        ax.contourf(LAT,LON,check.sel(zlevel=i).values,
            offset=i,cmap='Reds',alpha=.8,levels=[0,0.025])
plt.show()
#%%
#
