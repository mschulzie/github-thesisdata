...
for ort in sections:
    ...
    for file in dir:
        ...
        cl_max.append(ds['CHL_maximum'].mean(dim=('lon','lat')).values[0])
        cl_min.append(ds['CHL_minimum'].mean(dim=('lon','lat')).values[0])
        cl_std.append(ds['CHL_standard_deviation'].mean(dim=('lon','lat')).values[0])
    ...
    ax.plot(t,cl_mean,label='Klima Mittelwert')
    #ax.plot(t,cl_max,label='max')
    ax.plot(t,cl_min,label='Klima Minimum')
    ax.fill_between(t,cl_mean-cl_std,cl_mean+cl_std,
        color='#0094ff',facecolor='#b0d9fb',label='Klima Std.abw.',
        alpha=.5)
    format_ax(ax)
    ...
    ax.plot(t,chl-cl_mean,color='#a22e4d' ,label='Abweichungen zu Klimamittel')
