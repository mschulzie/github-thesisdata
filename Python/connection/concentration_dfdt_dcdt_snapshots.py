...
chl_max = 3
iron_min = 1e-2#iron.min().values
norm = SymLogNorm(2e-1,base=10,vmin=-chl_max,vmax=chl_max)
norm_iron = SymLogNorm(2e-1,base=10,vmin=-1,vmax=1)
cbformat = '%.1f'
cbticks = [-3,-2,-1,-.5,-.2,-.1,0,.1,.2,.5,1,2,3]

add=''
chl_label = '\nVeränderung Chl-a in mg/m3 pro Tag'
iron_label = 'Veränderte Eisenkonzentration in nM'
iron_raw = xr.open_dataarray('D://thesisdata/wrf_dust/fe_dep_advection_land_source_0_nM.nc')
iron_raw = iron_raw.sel(time=slice('2009-09-18T12','2009-10-01T12'))[::8] # MITTAGS!!!
iron_raw = iron_raw.assign_coords(time=pd.date_range('2009-09-18','2009-10-01',freq='d'))
...
iron_raw.values = iron_raw.values - iron_raw.values.mean(axis=0)
iron = iron_raw.diff(dim='time')
...
if norm_std:
    ...
    cbformat='%d'
    iron.values = iron.values / iron.values.std(axis=0)
    iron_label = 'Veränderte Eisenkonzentration in nM (genormt mit Std.abw.)'
...
for i,j in enumerate(gs1):
    ...
    im1 = iron[i,...].plot(ax=ax,transform=crs.PlateCarree(),cmap='RdBu_r',
    add_colorbar=False)
...
cb1.set_label(iron_label)
cb2.set_label(chl_label)

plt.show()


fig.savefig('D://thesisdata/bilder/Python/wrf_chla/correlation/concentration_snapshot'+
