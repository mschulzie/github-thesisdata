    ...
    for i in range(1,123):
        for j in range(1,163):
            ...
            C[t,i,j] =  (
                        ...
                        -v[t-1,i,j] * dt/dy[t-1,i,j]*(C[t-1,i+v_add,j]-C[t-1,i+v_sub,j])
                        -C[t-1,i,j]* (dt/dx[t-1,i,j]*(u[t-1,i,j+u_add]-u[t-1,i,j+u_sub])
                        +(dt/2/dy[t-1,i,j]*(v[t-1,i+v_add,j]-v[t-1,i+v_sub,j])))
                        )
    if source > 0:
...
v_sub
C_Fe.to_netcdf('D://thesisdata/wrf_dust/fe_dep_advection_land_source_FLUSS.nc')
