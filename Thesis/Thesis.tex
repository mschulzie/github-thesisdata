...
\end{figure}
Um die Entwicklung des Phytoplanktons genauer zu untersuchen, werden im folgenden Zeitreihen der Chlorophyll-a-Konzentrationen untersucht. Entsprechend Kapitel \ref{sec:timeseries} werden die Mittelwerte für die in Abbildung \ref{fig:iron_deposition_sections} gezeigten 5 verschiedenen Regionen gebildet und deren Zeitreihen für den Zeitraum Juni bis Dezember 2009 präsentiert. Daneben wird dieselbe Zeitreihe um den jahreszeitlichen Trend bereinigt und gleichzeitig die Region auf die Gitterpunkte beschränkt, für die durch das WRF-Modell einen totalen Staubeintrag von mehr als 5 µg pro Quadratmeter simuliert. Beide Maßnahmen helfen, ein möglicherweise mit den Staubdepositionen verbundenes Signal zu identifizieren. Ist das entsprechend bearbeitete Signal deutlicher als jenes für die gesamte Region, wäre dies sowohl für die korrekte Simulation der Staub- bzw. Eisendepositionen als auch einen etwaigen Zusammenhang ein positives Indiz.

\begin{figure}
\includegraphics[width=\textwidth]{bilder/timeseries_all.png}
\caption{Zeitreihe der mittleren Chlorophyll-a Konzentrationen für die komplette Domäne.} \label{fig:timeseries_full}
\end{figure}
