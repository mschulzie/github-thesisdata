...
Um den statistischen Zusammenhang zweier Variablen zu untersuchen, werden hier zwei verschiedene Verfahren angewendet. Zunächst wird anhand einer Zeitreihenanalyse der Konzentrationen des Phytoplanktons (Kapitel \ref{sec:timeseries}) überprüft, ob im Untersuchungszeitraum nach dem Staubsturm überhaupt eine nennenswerte Reaktion beobachtet werden kann. Aufgrund der regelmäßig und insbesondere saisonal schwankenden Veränderungen müssen dabei entsprechende Trends (welche unabhängig von der Staubdeposition zu Veränderungen führen) eliminiert werden. Für dieses Verfahren wird die Staubdeposition als \textit{ein} Ereignis betrachtet, welches in bestimmten Regionen Auswirkungen nehmen kann (nämlich in denen mit Depositionen größer einer entsprechenden Signifikanzschwelle laut WRF-Simulation). Tatsächlich liegen die Staubdepositionen durch die WRF-Simulation aber auch im Rahmen einer Zeitreihe vor. Unter der Annahme, dass eine potentielle Entwicklung des Phytoplanktons immer nach einer bestimmten Reaktionszeit stattfindet, ist eine Kovarianzanalyse der Zeitreihen der Depositionen und Phytoplanktonkonzentrationen (entsprechend um die Reaktionszeit verschoben) möglich. Hierzu wird in Kapitel \ref{sec:mca} die Maximum Kovarianzanalyse beschrieben.
\subsubsection{Zeitreihen- und Trendanalyse} \label{sec:timeseries}
Für die Zeitreihenanalyse werden alle Gitterzellen betrachtet, für die von der WRF-Simulation ausreichend hohe Eisendepositionen simuliert wurden. Um das Gebiet sinnvoll einzugrenzen, soll der gesamte Eintrag für die jeweilige relevante Zelle so groß sein, dass dadurch die \textit{normalen} Eisenkonzentrationen nennenswert erhöht werden. Auf Basis von Abbildung \ref{fig:nutrient_iron} wird angenommen, dass diese für den überwiegenden Teil der zu untersuchenden Region mindestens etwa 0.066 nM (Nanomol pro Liter) betragen. Darüber hinaus wird zur Abschätzung der Erhöhung der Eisenkonzentrationen $\text{Fe}_{c}$ durch Deposition angenommen, dass der überwiegende Teil der Nährstoffe in den oberen $z_0 = 10$m des Oberflächenwassers konsumiert wird. Mit den $M_{\text{Fe}} =  55.845$ g/mol Molmasse des Elements Eisen ergibt sich dann ausgehend der vom Modell simulierten Eisendeposition $Fe_{in}$ in g/qm:
\begin{equation}
\text{Fe}_{c} = \frac{\text{Fe}_{in}}{z_0 \cdot M_{\text{Fe}}} \Leftrightarrow \text{Fe}_{in} =  \text{Fe}_{c} \cdot z_0 \cdot M_{\text{Fe}}
\end{equation}
Um eine Erhöhung der Konzentrationen in der Größenordnung von mindestens 0.01 nM zu erreichen, müssen dementsprechend wenigstens etwa $5.6$ µg Eisen pro Quadratmeter eingetragen werden. Zum Vergleich: In den \textit{Bottle Experiments} von \citet{Martin.1988} wurden in verschiedenen Behältern 1 nM, 5 nM und 10 nM hinzugefügt. Die Mengen waren dort also um mindestens zwei Größenordnungen höher als in dieser Annahme. Im SOIREE-Experiment für den südlichen Ozean \citep{Trull.2001} konnten Veränderungen ab einer Schwelle von etwa 0.2 nM Eisen beobachtet werden, was einer Verdopplung der dortigen regulären Konzentration von 0.1 nM entspricht \citep{Boyd.2010}. Auch im Vergleich dazu ist der hier verwendete Schwellenwert eher optimistisch, da heißt, die Depositionen reichen möglicherweise nicht aus, um eine Reaktion des Phytoplanktons zu bewirken. \\
Da die chemischen Zusammensetzungen und Bedingungen für die Regionen unterschiedlich sein können (QUELLE ERGÄNZEN), sollen die anhand des Schwellenwertes identifizierten Zellen in verschiedene Sektoren eingruppiert werden, um diese jeweils separat zu analysieren. Für jeden Sektor $k$ wird anschließend für jeden verfügbaren Zeitpunkt der Mittelwert der Chlorophyll-a-Konzentrationen $C_i$ aus den $N_k$ dort betroffenen Zellen berechnet. Damit ergibt sich eine Zeitreihe für die Entwicklung des Phytoplanktons im gesamten Sektor $P_k(t)$:
\begin{equation}
P_{k}(t) = \frac{1}{N_k}\sum\limits_{i=1}^{N_k} C_i(t)
\end{equation}
Unter der Annahme, dass der Eintrag von Eisen einen Einfluss auf diese Zeitreihe nimmt, sollte diese nach dem Ereignis ansteigen. Dies wäre ein entsprechendes Indiz. Allerdings unterliegt die Entwicklung der Phytoplankton-Konzentrationen ohnehin (so auch im tasmanischen Meer \citep{Tilburg.2002}) einem jahreszeitlichen Gang sowie weiteren externen Einflüssen, die eine hohe Variabilität aufweisen können (vgl. Kapitel \ref{sec:nährstoffe}). Da der jahreszeitliche Trend aus den Klimadaten relativ gut bekannt ist, kann die Zeitreihe im Untersuchungszeitraum 2009 um diesen Effekt bereinigt werden. Hierzu werden Datensätze des \textit{Marine Copernicus Service} verwendet, welche die nötigen täglichen Klimadaten bereits liefern. Neben dem Mittelwert der Chlorophyll-a-Konzentrationen des jeweiligen Tages für die Jahre 1998 bis 2018 kann außerdem aus den 3\%, 16\%, 84\% und 97\% Perzentilen die entsprechende Variabilität abgeleitet werden. Der Klima-Mittelwert $C_{cli,i}$ einer jeden Gitterzelle wird ebenfalls über die jeweilige Sektion gemittelt
\begin{equation}
P_{cli,k}(t) = \frac{1}{N_k}\sum\limits_{i=1}^{N_k} C_{cli,i}(t)
\end{equation}
 und dient zur Entfernung eines möglichen Trends im zu untersuchenden Sektor, sodass ausschließlich die Abweichung/Anomalie $P_{a,k}$ zum Klimamittel dargestellt wird:
\begin{equation}
P_{a,k}(t) = P_{k}(t) - P_{cli,k}(t)
\end{equation} 
Im Fall eines düngenden Effektes der Eisendpositionen sollte diese bereinigte Zeitreihe nach den Depositionen einen steigenden Trend aufweisen. Um einzuschätzen, ob die Abweichungen überhaupt relevant sind, werden diese mit der Perzentil-Standardabweichung (16\%/84\%) sowie den 3\% und 97\% verglichen. Liegt die Anomalie unter der durch die Perzentil-Standardabweichung gegebenen Schwelle, ist ein Zusammenhang eher unwahrscheinlich, die Abweichungen könnten aus der gewöhnlichen, Staub-unabhängigen Variabilität resultieren. Liegen diese im Gegensatz deutlich darüber oder erreichen sogar das 97 \% Perzentil, handelt es sich um sehr seltene Ereignisse (in 97\% der Fälle liegen die Konzentrationen/Abweichungen) unter diesem Wert), bei denen ein externer Einfluss wahrscheinlich ist Kann dieser in unserem Fall in einen zeitlichen Zusammenhang mit den Staubeinträgen gebracht werden, wäre das ein deutlichen Indiz für einen Einfluss ebendieser. Auch die Perzentliche liegen je Gitterzelle vor und es wird entsprechend für den jeweiligen Sektor $k$ der Mittelwert dieser Variationen gebildet.
